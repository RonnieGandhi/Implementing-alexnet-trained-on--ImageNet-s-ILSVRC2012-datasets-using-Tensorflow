...
def batch_norm(x):
  ...
  return tff.nn.batch_normalization(x,batch_mean,batch_var,None,None,epsilon)
...
def read_image(images_folder):
  """
  It reads a single image file into a numpy array and preprocesses it
...
def preprocess_image(image_path):
  """
  It reads an image , it resize it to have the lowest dimension of it to have lowest dimension of 256px,
    it randomly choose a 224x224 crop inside the resized image and normalize the numpy
    ...
    Returns:
          cropped_im_array: the numpy of the image normalized[width,height,channels]
  """
...
def read_k_patches(image_path,k):
    ...
    img = cv2.imread(image_path)
    height,width,_ = img.shape
#   if self.horizaontal and np.random.random()<0.5:
  # img = cv2.flip(img,1)
    #resize of the image(setting lowest dimension to 256px)
...
def read_batch(batch_size,image_source,wind_labels):
  """It returns a batch of single images(no data-augmentation)
         ILSVRC 2012 training set folder should be structured like this:
         ILSVRC2012_img_train 
          ...
          |_...
           ...
           Args:
                  batch_size: need explanation? xD
  ...
  for i in range(batch_size):
        # random class choice
        # (randomly choose a folder of image of the same class from a list of previously sorted winds)
...
def load_imagenet_meta(meta_path):
      """It reads ImageNEt metadata from ILSVRC 2012 dev tool file
            ...
            Returns:
                wnids: list of ImageNet labels(as strings)
                words: list of words(as strings) referring to winds labels and describing the classes
      """
      metadata = loadmat(meta_path,struct_as_record = False)
      
      #['ILSVRC2012','WNID','words','gloss','num_children','children','wordnet_height','num_train_images']
...
def read_test_labels(annotations_path):
      """
      It reads groundthruth labels from ILSVRC 2012 annotations file
              Args:
                      annotations_path: path to the annotations file
              Returns:
                      gt_labels: a numpy array vector of onehot labels
      """
      gt_labels = []
...
def imagenet_size(im_source):
  ...
  """It calculates the number of examples in ImageNet training-set
            Args:
                    im_source:path to ILSVRC2012 training set folder
            Returns:
  ...
  for d in os.listdir(im_source):
    for f in os.listdir(os.path.join(im_soruce,d)):
      n+=1
